...
const (
	...
	ErrNilCallback       = ErrorType("indexmempool: callback is nil")
	ErrNotAllocatedIndex = ErrorType("indexmempool: not allocated index")
...
//   - 할당되지 않은 인덱스(중복 반납 포함)면 ErrNotAllocatedIndex를 반환합니다.
...
func (ip *Pool[T]) Access(index int, fn func(*T)) error {
	...
	c.index, c.action, c.slotIndex, c.fn = -1, ActionAccess, index, fn
	c.Next()
	return c.err
}

// AccessLock은 할당된 슬롯의 메모리 포인터를 콜백 fn에 전달합니다. (블로킹 방식)
//
...
func (ip *Pool[T]) AccessLock(index int, fn func(*T)) error {
	...
	c.index, c.action, c.slotIndex, c.fn = -1, ActionAccessLock, index, fn
	c.Next()
	return c.err
}
