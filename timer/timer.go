...
func (eng *Engine[T]) Close() {
	...
	// 만료에는 성공했으나 아직 소비되지 않은 키는 pq.Close()에서 폐기되므로 유실로 집계한다.
	// 집계하지 않으면 이 경로의 유실이 어디에도 드러나지 않는다.
	// pq.Close() 이후에는 내부 카운터가 0으로 초기화되므로 반드시 그 전에 읽어야 한다.
	eng.qFail.Add(int64(eng.pq.Len()))
...
// QFail은 만료되었으나 C()로 전달되지 못한 키의 누적 개수를 반환합니다.
//
// 유실 경로는 두 가지이며 둘 다 이 값에 포함됩니다.
//   - 만료 큐에 넣지 못한 경우. 정원 검사가 만료 큐 용량과 같은 값을 쓰므로 정상 운영 중에는
//     큐가 넘치지 않고, 실질적으로 Close 이후 만료된 타이머가 여기 해당합니다.
//     (Close는 대기 중인 타이머를 취소하지 않습니다. 자세한 내용은 Close 참고)
//   - 만료 큐에는 들어갔으나 소비되기 전에 Close되어 폐기된 경우. Close 시점의 잔여 개수를 더합니다.
//
// 후자를 Close 시점에 한 번에 세므로, 하필 그 순간 수신자가 도착해 마지막 1건이 전달되면
// 실제 유실보다 1 크게 보고될 수 있습니다. 오차는 Close 1회당 최대 1건입니다.
