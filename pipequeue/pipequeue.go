...
//
// [남아 있는 데이터는 폐기됩니다]
// Close 시점에 큐에 남아 있던 데이터는 사실상 전부 C()로 전달되지 못하고 버려집니다.
// pipe 고루틴이 Close 이후 남은 항목을 꺼내더라도 write가 closeSig를 먼저 확인해
// 그대로 폐기하므로, 버퍼에 있던 항목이 전달되는 일은 없습니다.
//
// 다만 전달을 절대 금지하지도 않습니다. Close 시점에 이미 출력 채널 송신을 대기 중이던
// 항목 1건은, 하필 그 순간 수신자가 준비되면 폐기와 전달 중 무엇이 선택될지 정해지지 않습니다.
// 어느 쪽이든 Put이 nil(성공)을 반환했더라도 소비 이전에 Close되면 그 데이터는 유실된다고 가정하십시오.
//
// 폐기를 기본으로 택한 이유는 고루틴 누수를 막기 위해서입니다. 남은 데이터를 끝까지 전달하려면
// 출력 채널을 비워 줄 수신자가 필요한데, 종료 시점에 수신자가 이미 사라졌다면
// pipe 고루틴이 송신 지점에서 영원히 대기하며 누수됩니다.
// 유실 가능성을 감수하더라도 즉시 종료해 누수를 막는 쪽을 택했습니다.
//
...
// 소비 이전에 Close가 호출되면 해당 데이터는 유실됩니다. (자세한 내용은 Close 참고)
...
// 닫힌 큐는 더 이상 변경되지 않으므로, Close 이후의 등록은 조용히 무시됩니다.
//
